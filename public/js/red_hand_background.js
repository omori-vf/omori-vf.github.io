...
class RedHand {
    ...
    static get_random_position() {
        return new Vector2D(random_integer(0, canvas.width), random_integer(0, canvas.height));
    }

    static get_random_target() {
        return new Vector2D(random_integer(RedHand.target_margin, canvas.width - RedHand.target_margin), random_integer(RedHand.target_margin, canvas.height - RedHand.target_margin));
    }
    ...
    update_animation(elapsed_time) {
        ...
        if (this.animation_time > RedHand.animation_update_time_rate) {
            this.animation = this.animation == RedHand.animation_1 ? RedHand.animation_2 : RedHand.animation_1;
            this.animation_time = 0;
        }
    }
    ...
    draw() {
        if (this.position.x < -RedHand.animation_width || this.position.x > canvas.width + RedHand.animation_width || this.position.y < -RedHand.animation_height || this.position.y > canvas.height + RedHand.animation_height) // don't bother drawing if off screen
            return;

        const angle = Math.atan2(this.velocity.y, this.velocity.x) + Math.PI;

        ctx.save();
        ctx.translate(this.position.x + RedHand.animation_width / 2, this.position.y + RedHand.animation_height / 2);
        ctx.rotate(angle);
